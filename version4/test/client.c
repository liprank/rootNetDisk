...
static int userLogin1(int sockfd, train_t *pt)
{
    ...
    while(1) {
        ...
        if(t.type == CMD_TYPE_USERNAME_RESP_ERROR) {
            printf("\033[1;31m[root]\033[0m 用户名不存在，请重新输入\n");
            continue;
        }
		...
		//存储用户名，放入client_user_info
		strcpy(clientUser->username,t.buff);
...
static int userLogin2(int sockfd, train_t * pt)
{
    ...
    while(1) {
        ...
        if(t.type == CMD_TYPE_ENCODE_RESP_ERROR) {
            //密码不正确
            printf("\033[1;31m[root]\033[0m 密码不正确，请重新输入\n");
            continue;
        } else {
            ret = recvn(sockfd, t.buff, t.len);

            printf("\033[1;31m[root]\033[0m Login Success.\n");
			//接收客户端加密过的token
			bzero(&t,sizeof(t));
			recvn(sockfd,&t.len,4);
			recvn(sockfd,&t.type,4);//此类型无用
			recvn(sockfd,&clientUser->token,t.len);//接收token
			//printf("token:%s\n",clientUser->token);

            printf("\033[1;31m[root]\033[0m please input a command.\n");
...
static int userEnroll1(int sockfd, train_t *pt){
	...
	while(1){
		...
		if(t.type == CMD_TYPE_USERNAME_RESP_ERROR_EN){
			printf("\033[1;31m[root]\033[0m 用户名已存在，请重新输入\n");
			continue;
		}else if(t.type == CMD_TYPE_USERNAME_RESP_OK_EN){//TODO:server 是在哪查的？
			//用户名不存在，进行注册
			//TODO:打开mysql，注册用户表
			printf("\033[1;31m[root]\033[0m 用户名不存在，可以注册\n");
			break;
		}
	}
}
...
static int userEnroll2(int sockfd, train_t *pt){
	...
	while(1){
		...
		if(t.type == CMD_TYPE_ENCODE_RESP_ERROR_EN){
			printf("\033[1;31m[root]\033[0m 密码不足8位，请重新设置\n");
			continue;
		}else {
			printf("\033[1;31m[root]\033[0m 注册成功.\n");
			
			//将token写入clientuser结构体
			bzero(&t,sizeof(t));
			recvn(sockfd,&t.len,4);
			recvn(sockfd,&t.type,4);//此类型无用
			recvn(sockfd,&clientUser->token,t.len);//接收token
