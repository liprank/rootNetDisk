...
int insert_into_files(MYSQL *pcoon, int parentId, int ownerId, const char *fileName) {
    const char *query_file = "INSERT INTO files (parentId, ownerId, fileName, fileType, fileSize, modifyTime) VALUES (?, ?, ?, 'd' ,4096, NOW())";
    MYSQL_STMT *stmt_file = mysql_stmt_init(pcoon);
    if (stmt_file == NULL) {
