...
void cdCommand(task_t * task)
{
    ...
    if(strcmp(task->data,"..") == 0){
        if(parentId == 0){
            ...
            char curdirectory[2048] = {0};
            printf("fileName = %s, wonerId = %d\n", task->data, ownerId);
            sprintf(curdirectory,"update users set curdirectory = %d where userId = %d",parentId,userId);
            printf("curdirectory = %s\n",curdirectory);
            int ret = mysql_query(task->pconn, curdirectory);
            printf("ret = %d",ret);
            if(ret != 0){
                char err[100] = {0};
        ...
        sprintf(curdirectory,"update users set curdirectory = %d where userId = %d",parentId,userId);
        printf("curdirectory = %s\n",query);
        ret = mysql_query(task->pconn, curdirectory);
        printf("ret = %d",ret);
        if(ret != 0){
            char err[100] = {0};
            sprintf(err,"(%d, %s)\n",mysql_errno(task->pconn), mysql_error(task->pconn));
